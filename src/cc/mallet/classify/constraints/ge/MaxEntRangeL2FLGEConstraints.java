...
public class MaxEntRangeL2FLGEConstraints implements MaxEntGEConstraint {
  ...
  public void computeExpectations(FeatureVector input, double[] dist, double weight) {
    preProcess(input);
      for (int li = 0; li < numLabels; li++) { // thk22, numLabels is too much here, so we fail with an ArrayIndexOutOfBoundsException
  ...
  public void zeroExpectations() {
    for (int fi : constraints.keys()) {
     ...
     constraints.get(fi).expectation = new double[constraints.get(fi).getNumConstrainedLabels()];
        //thk22!!! Crashing in computeExpecations because getNumConstraintedLabels is 1, but in computeExpectations the loop goes until numLables which may be larger than getNumConstrainedLabels, bloody idiot!!!!!!!!    constraints.get(fi).expectation = new double[numLabels];
    }
  }
